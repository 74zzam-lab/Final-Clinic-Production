  ...
  function renderOwnerHubPage() {
    ...
    const dailyKpis = `
      <div class="oh-grid">
        ...
        <div class="oh-card"><h4>فرع الجلسة</h4><div class="oh-val" style="font-size:15px">${global.BranchScope?.getActiveBranchId?.() || m.lockedBranch}</div><div class="oh-muted">${canSwitch ? 'يمكنك التبديل بين الفروع (Owner / HQ Admin)' : 'فرع هذا الجهاز — محدد عند سحب الترخيص'}</div></div>
