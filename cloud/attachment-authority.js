...
(function (global) {
  ...
  function migrateLegacyIfNeeded() {
    ...
    saveRecords(records, { source: 'legacy_manifest_migrate', skipBranchMerge: true, skipBranchGate: true });
  ...
  function saveRecords(records, options) {
    ...
    if (global.Repository?.setAll && global.Repository.isSyncedTable?.(META_TABLE)) {
      global.Repository.setAll(META_TABLE, merged, {
        branchId: bid,
        source: options.source || 'attachment_authority',
      });
      try { global.DB?.__rawSet?.(META_TABLE, merged); } catch {
        try { global.DB?.set?.(META_TABLE, merged); } catch { /* empty */ }
      }
      return { ok: true, count: merged.length, via: 'repository' };
    }

    if (global.SqliteBridge?.setAuthoritative) {
      return Promise.resolve(global.SqliteBridge.setAuthoritative(META_TABLE, merged)).then((r) => ({
        ok: !r || r.ok !== false,
        count: merged.length,
        via: 'sqlite_bridge',
        res: r,
    ...
    try { global.DB?.set?.(META_TABLE, merged); } catch { /* empty */ }
    return Promise.resolve({ ok: true, count: merged.length, via: 'db_fallback' });
  }

  async function saveItems(items, options) {
    const records = (items || []).map(itemToMetaRecord).filter(Boolean);
    const res = saveRecords(records, options);
