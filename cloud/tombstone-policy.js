/**
 * Renderer mirror of database/tombstone-policy.js
 */
(function (global) {
  'use strict';
  ...
  function tombstoneToMergeAction(tombDecision) {
    if (!tombDecision) return null;
    const MP = global.MergePolicy?.ACTIONS || ACTIONS;
    switch (tombDecision.action) {
      case ACTIONS.SKIP:
        return { action: MP.SKIP || 'skip', reason: tombDecision.reason };
      case ACTIONS.PUSH:
        return { action: MP.PUSH || 'push', reason: tombDecision.reason };
      case ACTIONS.PULL:
        return { action: MP.PULL || 'pull', reason: tombDecision.reason };
      case ACTIONS.CONFLICT:
        return {
          action: MP.CONFLICT || 'conflict',
