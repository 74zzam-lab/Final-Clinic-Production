...
check(/licenseSnapshot/.test(bootstrapSrc), 'bootstrap accepts licenseSnapshot fallback');
...
check(/credentialSkip|users\.credentials/.test(indexSrc), 'import UI handles credential skip intentionally');
...
bootstrapMod.configure({
  ...
  readLicense: () => ({ ok: false, missing: true }),
  getSession: () => null,
});
...
(async () => {
  ...
  check(issued.ok && issued.capabilityId, 'bootstrap issues cap with licenseSnapshot when cache empty');
